    ...
    <style>
        :root {
            ...
            --my_students_details-badge-cancel-text: #7C4633;
            --my_students_details-badge-trial-bg: #BFE1CD;
            --my_students_details-badge-trial-border: #A9CFBB;
            --my_students_details-badge-trial-text: #2E6B4A;
...
<div class="my_students_details_container">
    ...
    <script>
        ...
        // ================= UTILITIES =================
        const my_students_details_percent = (done, total) => total === 0 ? 0 : Math.round((done / total) * 100);

        function my_students_details_makeAvatar(person) {
            const img = person.avatarUrl ? `<img src="${person.avatarUrl}" alt="${person.name}">` : `<span>${person.avatar}</span>`;
        ...
        // ***** UPDATED: pass student name so kebab has data-student and menu can show it *****
        function my_students_details_iconsCell(studentName) {
            return `
        <div class="my_students_details_icon-row">
          <button class="my_students_details_icon-btn" aria-label="Message"><img src="img/my_students/chat.svg" alt="Chat" class="my_students_details_icon-img"/></button>
          <div class="my_students_details_calendar-wrap">
            <button class="my_students_details_icon-btn" aria-label="Schedule"><img src="img/my_students/calendar.svg" alt="Calendar" class="my_students_details_icon-img"/></button>
          </div>
          <button class="my_students_details_icon-btn my_students_details_settings_menu_kebab" data-student="${studentName}" aria-label="More"><img src="img/my_students/settings.svg" alt="More" class="my_students_details_icon-img"/></button>
        </div>`;
        }

        function my_students_details_row(data) {
            return `
        ...
        function my_students_details_render(rows) {
            ...
            $wrap.empty();
            rows.forEach(r => $wrap.append(my_students_details_row(r)));
        }
        ...
        $(function() {
            ...
            $('#my_students_details_search_input').on('input', function() {
                ...
                const filtered = my_students_details_seed.filter(s => s.name.toLowerCase().includes(q));
            ...
            $('#my_students_details_sort_lessons').on('click', function() {
                ...
                $sortIcon.css('transform', my_students_details_lessonsAsc ? 'rotate(180deg)' : 'rotate(0deg)');
            ...
            $('.my_students_details_check').on('click', function() {
                ...
                $(this).closest('.my_students_details_filter_item').toggleClass('is_active', $(this).hasClass('is_checked'));
            });
        });

        // Clear & Apply (outside DOM ready in your original)
        $('#my_students_details_btn_clear').on('click', function() {
            $('.my_students_details_check').removeClass('is_checked').closest('.my_students_details_filter_item').removeClass('is_active');
        });
        ...
        function my_students_details_settings_menu_open(btn) {
            ...
            const top = rect.bottom + 8 + window.scrollY;
            let left = rect.right - my_students_details_settings_menu_$panel.outerWidth() + window.scrollX; // right-align
        ...
        // open on existing settings buttons
        $(document).on('click', '.my_students_details_settings_menu_kebab', function(e) {
            e.stopPropagation();
            if (my_students_details_settings_menu_$panel.is(':visible') &&
        ...
        // item click placeholder
        $('#my_students_details_settings_menu_panel').on('click', '.my_students_details_settings_menu_item', function() {
            const action = $(this).data('action');
            const name = my_students_details_settings_menu_currentTarget ? $(my_students_details_settings_menu_currentTarget).data('student') : '';
