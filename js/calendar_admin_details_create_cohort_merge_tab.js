$(".calendar_admin_details_create_cohort_tab").click(function () {
  ...
  if (!["cohort", "conference", "peertalk", "merge"].includes(tab)) {
    $(
      "#mainModalContent, #conferenceTabContent, #peerTalkTabContent, #mergeTabContent"
    ).hide();
  }
});
...
$(document).on("click", ".merge-calendar-day", function () {
  ...
  mergeSelectedCalendarDate = new Date(
    mergeCalendarMonth.year,
    mergeCalendarMonth.month,
    day
  );
...
function mergeRenderCalendarModal() {
  ...
  for (let d = 0; d < 7; d++)
    html += `<div class="merge-calendar-day-header">${dayHeaders[d]}</div>`;
  ...
  for (let i = 0; i < prevMonthDays; i++)
    html += `<div class="merge-calendar-day-inactive"></div>`;
  for (let d = 1; d <= totalDays; d++) {
    let sel =
      mergeSelectedCalendarDate &&
      mergeSelectedCalendarDate.getFullYear() === y &&
      mergeSelectedCalendarDate.getMonth() === m &&
      mergeSelectedCalendarDate.getDate() === d
        ? " selected"
        : "";
...
// ✅ Merge Cohort form validation
$("#mergeCohortForm").on("submit", function (e) {
  e.preventDefault();
  ...
  const closingCohort = $("#mergeClosingCohortSelected").text().trim();
  const joiningCohort = $("#mergeJoiningCohortSelected").text().trim();
  const closingDateBtn = $("#mergeClosingDateBtn");
  const mergingDateBtn = $("#mergeMergingDateBtn");
  const closingDate = closingDateBtn.text().trim();
  const mergingDate = mergingDateBtn.text().trim();

  // validate Closing Cohort
  if (!closingCohort || closingCohort === "FL-6-XXXXXX-0092") {
    $("#mergeClosingCohortBtn").addClass("field-error");
    isValid = false;
  }

  // validate Joining Cohort
  if (!joiningCohort || joiningCohort === "FL-6-XXXXXX-0092") {
    $("#mergeJoiningCohortBtn").addClass("field-error");
    isValid = false;
  }

  // validate Closing Date
  if (!closingDate || closingDate === "Select Date") {
    closingDateBtn.addClass("field-error");
    isValid = false;
  }

  // validate Merging Date
  if (!mergingDate || mergingDate === "Select Date") {
    mergingDateBtn.addClass("field-error");
    isValid = false;
  }
  ...
  // ✅ Passed validation — build payload
  const payload = {
    closingCohort,
    joiningCohort,
    closingDate,
    mergingDate,
    closeNow: $("#mergeNow").is(":checked"),
    mergeNow: $("#mergeNow").is(":checked"),
    submittedAt: new Date().toISOString(),
  };

  console.log("✅ Merge Cohort payload:", payload);
});
